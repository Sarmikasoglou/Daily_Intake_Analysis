# Cow Intake and Body Weight Visualizer
...
- Upload a body-weight workbook in a separate tab and link `EID` to `EART`.
- Upload a treatment arrangement file with `EART` and `Treatment` for the Body Weights tab.
...
- In the Body Weights tab, the uploaded workbook should contain `EID`, `Date` or `timestamp`, and `Weight (Kg)` or `weight`. The weight-file `EID` is matched to the lookup-file `EID`, and the displayed cow identifiers come from `EART`, `EID`, and `TransID`.
- The Body Weights tab also accepts a treatment arrangement file with `EART` and `Treatment`, so you can filter body weights to the uploaded treatment list and plot average body weight by treatment.
